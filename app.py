...
if buscar and titulo_libro:
    ...
    if titulo_libro_normalizado not in libros_normalizados:
        ...
        with st.spinner("Analizando el libro..."):
            st.session_state['analisis'] = analizar_libro(titulo_original)
            st.session_state['titulo_original'] = titulo_original
            st.session_state['ficha_path'] = generar_ficha(titulo_original, st.session_state['analisis'])
            st.session_state['rubrica_path'] = generar_rubrica(titulo_original)
            st.session_state['recomendaciones'] = recomendar_libros(titulo_original, libros_disponibles)
...
if 'analisis' in st.session_state:
    ...
    with open(st.session_state['ficha_path'], "rb") as f:
        st.download_button("📥 Descargar ficha", f, file_name=st.session_state['ficha_path'])

    st.subheader("📊 Rúbrica de evaluación")
    with open(st.session_state['rubrica_path'], "rb") as f:
        st.download_button("📥 Descargar rúbrica", f, file_name=st.session_state['rubrica_path'])
